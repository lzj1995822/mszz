            ...
            <el-tab-pane label="基本信息" name="first">
              <el-col id="KeyDriverInfo"  :span="24">
               <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;" width="100%" cellspacing="0">
                   ...
                   <tr v-for="(v,k,i) in form" :key="i" v-if="k!=='spEventID'&&k!=='currentResponsiblePersonCertID'&& k!=='currentResponsiblePersonName'&&k!=='currentResponsiblePersonContact'">
                       <td class="tb_label">{{labels[i]}}</td>
                       <td>
                           <span v-if="k=='spEventKind'">
                               {{getEventKind(v)}}
                           </span>
                           <span v-else>
                                {{v}}
                   ...
                   <template v-for="(k,i) of SAPerson">
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" :key="z" v-if="y!=='spEventID'&&y!=='spEPersonsID'">
                 ...
                 <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;width:100%;height:100%"  cellspacing="0">
                    ...
                    <tr v-if="DealingBasic&&DealingBasic.length>0">
                     ...
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" :key="z" v-if="y!=='speventID'&&y!=='skynetPatrolEventDealingID'">
                   ...
                   <tr v-if="Check.length==0&&DealingBasic.length==0&&Distribute.length==0">
                    ...
                    <template v-for="(v,i) of Distribute">
                      ...
                      <tr v-for="(x,y,z) of v" :key="z" v-if="y!=='speventID'&&y!=='skynetPatrolEventDistributeID'">
...
export default {
  data() {
    return {
      ...
      labels: ["编号","事件名称","事件性质","事件类型", "事件内容","事件开始时间",
      "事件结束时间","经度","纬度","事件最新处理状态","当前备注","当前经办人员身份证号","当前经办人员姓名","经办人员联系方式","网格",'上报人','部门','电话'],
      label:['编号','事故编号','当事人身份证','当事人姓名','备注','当事人联系方式'],
      label2:['编号','姓名','处理人','处理日期','处理内容','备注','事故编号'],
      label3:['编号','审核人','所属部门','审核日期','审核结果','备注','事故编号'],
      label4:['编号','派发人姓名','所属部门','派发对象姓名','所属部门','开始时间','结束时间','备注','事故编号'],
      pics:[],
      video:{
          id:1,
          url:''
  ...
  watch:{
    id:function(val){
      api.getSPDetail(this.id).then(res=>{
        this.form = res.data.SPEventBasic
        this.pics = res.data.SPEventMedia.pictures.length>0?res.data.SPEventMedia.pictures:''
        this.video = res.data.SPEventMedia.video.length>0?res.data.SPEventMedia.video[0]:''
...
// WEBPACK FOOTER //
// src/views/skynet/eventInfo.vue?21b30ccf