          ...
          <el-col :span="$store.getters.user.userRole=='超级管理员'?21:24">
            <el-row>
              ...
              <el-col :span="16">
                <div align="right">
                  ...
                  <el-button type="primary" @click="getCorrectioner">查询</el-button>
            ...
            <el-row>
              ...
              <el-col :span="24" style="margin-top:10px" align="right">
                <el-pagination layout="prev, pager, next" :current-page="currentPage" :total="totalInfo" @current-change="handleCurrentChange">
                </el-pagination>
              </el-col>
            </el-row>
          </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteById(id) {
        this.$confirm("是否删除此人记录, 是否继续?", "提示", {
          ...
          .then(() => {
            api.deleteSpecialPerson(id, "02").then(res => {
              if (res.data) {
                this.$message({
                  type: "success",
                  message: "删除成功!"
      ...
      getCorrectioner() {
        api.fetchCorrectionerPerson(this.filters).then(res => {
    ...
    mounted() {
      ...
      this.$store.commit("INIT_PAGE", "H");
      this.filters.page = this.$store.getters.page["H"];
...
// WEBPACK FOOTER //
// src/views/specialPopulationManage/communityCorrectionerPopulation.vue?cf3e5488