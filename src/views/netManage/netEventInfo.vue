            ...
            <el-tab-pane label="基本信息" name="first">
              <el-col id="KeyDriverInfo" :span="24">
               <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;" width="100%" cellspacing="0">
                   ...
                   <tr v-for="(v,k,i) in form" :key="i" v-if="k!=='eventID'&&k!=='currentResponsiblePersonCertID'&& k!=='currentResponsiblePersonName'&&k!=='currentResponsiblePersonContact'">
                   ...
                   <template v-for="(k,i) of SAPerson">
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" :key="z" v-if="y!=='eventID'&&y!=='spEPersonsID'">
                 ...
                 <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;width:100%;height:100%"  cellspacing="0">
                    ...
                    <tr v-if="DealingBasic&&DealingBasic.length>0">
                     ...
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" :key="z" v-if="y!=='eventID'&&y!=='eventDealingID'">
                   ...
                   <tr v-if="DealingMedia.pictures&&DealingMedia.pictures.length>0">
                     <td class="tb_label">图像资料</td>
                     <td>
                       <span v-for="(v,i) of DealingMedia.pictures" :key="i">
                         <img :src="v.url" width="300" alt="">
                       </span>
                     </td>
                   </tr>
                   ...
                   <tr v-if="DealingMedia.video&&DealingMedia.video.length>0">
                     <td class="tb_label">视频资料</td>
                     <td>
                       <span v-for="(v,i) of DealingMedia.video" :key="i">
                   ...
                   <template v-for="(k,i) of Check">
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" :key="z" v-if="y!=='eventID'&&y!=='eventCheckedID'">
                   ...
                   <tr v-if="Check.length==0&&DealingBasic.length==0&&Distribute.length==0">
                     <td style="text-align:center;color:#909399;font-size:30px">
                       <span >
                         事件未立案或尚未处理
                       </span>
                     </td>
                    ...
                    <template v-for="(v,i) of Distribute">
                      ...
                      <tr v-for="(x,y,z) of v" :key="z" v-if="y!=='eventID'&&y!=='eventDistributeID'">
...
export default {
  data() {
    return {
      form: {
       ...
       labels:["事件编号","事件名称" ,"事件性质", "事件类型",'事件详情',"事件开始日期","事件结束日期","事发经度","事发纬度",
      "事件处理状态","当前事件记录","当前经办人身份证号码", "当前经办人员姓名", "当前经办人员联系方式","所属网格","上报人","部门","联系方式"],
      label:['编号','事故编号','当事人身份证','当事人姓名','备注','当事人联系方式'],
      label2:['编号','事件编号','姓名',"部门","联系方式",'处理日期','处理内容','备注'],
      label3:['编号',"人员编号",'审核人','所属部门','审核日期','审核结果','备注','事故编号'],
      label4:['编号','人员编号','派发人姓名','所属部门','派发对象姓名','所属部门','开始时间','结束时间','备注'],
      pics:[],
      video:{
          id:1,
          url:''
  ...
  watch:{
    id:function (param) {
      api.getNetGridEventDetail(this.id).then(res=>{
        this.form = res.data.netGridEventDetailBasic
        this.pics = res.data.netGridEventMedia.pictures.length>0?res.data.netGridEventMedia.pictures:''
        this.video = res.data.netGridEventMedia.video.length>0?res.data.netGridEventMedia.video[0]:''
        this.SAPerson = res.data.involvedPersons

        this.DealingBasic = res.data.dealingBasic
        this.DealingMedia = res.data.dealingMedia
...
// WEBPACK FOOTER //
// src/views/netManage/netEventInfo.vue?35907f60