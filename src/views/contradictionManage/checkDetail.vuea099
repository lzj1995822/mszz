               ...
               <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;" width="100%" cellspacing="0">
                   ...
                   <template v-for="(k,i) of SAPerson">
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" v-if="y!=='eventID'&&y!=='spEPersonsID'">
                    ...
                    <tr v-if="DealingBasic&&DealingBasic.length>0">
                     ...
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" v-if="y!=='eventID'&&y!=='eventDealingID'">
                        ...
                        <td>
                          <span v-if="y=='isSuccessInSolving'">
                            {{x==1?'成功':'失败'}}
                          </span>
                          <span v-else>
                            {{x}}
...
export default {
  data() {
    return {
      form: {
       ...
       labels:["事件编号","事件名称" ,"发生日期","发生地点", "事件规模",'事件类型',"涉及人数","事件简述","涉及单位","事发经度","事发纬度",
      "事件处理状态","事发网格","上报人","部门","联系方式"],
      label:['编号','事故编号','当事人身份证','当事人姓名','备注','当事人联系方式'],
      label2:['编号','事件编号','姓名',"部门","联系方式",'处理日期','处理内容','化解方式','是否化解成功','备注'],
  ...
  watch:{
    id:function (param) {
      api.getDisputeDetail(this.id).then(res=>{
        this.form = res.data.disputeEventsBasic
        this.pics = res.data.disputeEventsMedia.pictures.length>0?res.data.disputeEventsMedia.pictures:''
        this.video = res.data.disputeEventsMedia.video.length>0?res.data.disputeEventsMedia.video[0]:''
        ...
        this.Distribute = res.data.distributeBasic
      })
    }
  },
  mounted() {
...
// WEBPACK FOOTER //
// src/views/contradictionManage/checkDetail.vue?ab22be40