            ...
            <el-table :data="list" border>
              ...
              <el-table-column label="操作" align="center">
                <template scope="scope">
                  <el-button
                    type="danger"
                    v-if="scope.row.eventStatus=='01'"
                    size="small"
                    @click="eventLiAn('03',scope.row.disputeEventID)"
                  >事件立案</el-button>
                  <el-button
                    type="primary"
                    v-if="scope.row.eventStatus=='01'"
                    size="small"
                    @click="eventLiAn('02',scope.row.disputeEventID)"
                  >不予立案</el-button>
                  <el-button
                    type="warning"
                    v-if="scope.row.eventStatus=='03'"
                    size="small"
                    @click="eventPaifa(scope.row.disputeEventID)"
                  >事件派发</el-button>
                  <!-- <el-button type="warning" size="small" @click="eventUp(scope.row.disputeEventID)">事件派发</el-button> -->
...
export default {
  ...
  methods: {
    ...
    eventLiAn(type, id) {
      let m =
        type == "03"
          ? "事件立案成功！等待派发！"
          : "事件取消立案，等待上报人处理。";
      api.updateDisputeEventStatus(type, id).then(res => {
        if (res.code == 200) {
          this.$message({
            type: "success",
            message: m
        ...
        this.$confirm("是否确认派发此事件,是否继续?", "提示", {
          ...
          .then(() => {
            api.addDisputeEventDistribute(this.form).then(res => {
              if (res.code == 200) {
                api
                  .updateDisputeEventStatus("04", this.form.eventID)
                  .then(res => {
                    if (res.code == 200) {
                      this.$message({
                        type: "success",
    ...
    getInfo() {
      api.getDisputeEventDespatcherQuery(this.filters).then(res => {
...
// WEBPACK FOOTER //
// src/views/contradictionManage/eventPaifa.vue?163b6441