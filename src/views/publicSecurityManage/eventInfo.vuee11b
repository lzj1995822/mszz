            ...
            <el-tab-pane label="基本信息" name="first">
              <el-col id="KeyDriverInfo" :span="24">
               <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;" width="100%" cellspacing="0">
                   ...
                   <tr v-for="(v,k,i) in form" :key="i" v-if="k!=='ssEventID'&&k!=='currentResponsiblePersonCertID'&& k!=='currentResponsiblePersonName'&&k!=='currentResponsiblePersonContact'">
                   ...
                   <template v-for="(k,i) of SAPerson">
                     <tr>
                      ...
                      <tr v-for="(x,y,z) of k" :key="z" v-if="y!=='ssEventID'&&y!=='spEPersonsID'">
...
export default {
  ...
  watch:{
    id:function (param) {
       api.getSSDetail(this.id).then(res => {
      this.form = res.data.SSEventBasic;
      this.pics =
        res.data.SSEventMedia.pictures.length > 0
          ? res.data.SSEventMedia.pictures
          : "";
      this.video =
        res.data.SSEventMedia.video.length > 0
          ? res.data.SSEventMedia.video[0]
          : "";
      this.SAPerson = res.data.involvedPersons;

      this.DealingBasic = res.data.dealings;
      this.DealingMedia = res.data.dealingsMedia;
      this.Check = res.data.checkeds;
      this.Distribute = res.data.distributes;
    });
    }
  },
  mounted() {
...
// WEBPACK FOOTER //
// src/views/publicSecurityManage/eventInfo.vue?d515168e