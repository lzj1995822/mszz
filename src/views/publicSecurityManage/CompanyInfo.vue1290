<template>
  <div>
      <el-row>
          <el-col  id="deliveryInfo"  :span="24">
            <table class="userInfoEditTable" style="border-collapse:collapse;background-color:white;" width="100%" cellspacing="0">
                <tr v-for="(value,k,index) in form" :key="index" v-if="k!=='isLogisticsEnterprise'&&k!=='isKeyEnterprise'">
                    <td class="tb_label">{{label[index]+':'}}</td>
                    <td>
                      <span v-if="k==='hasCPC'||k==='isDangeriousEnterprise'||k==='isCanConstructCPC'||k==='hasLabourUnion'||k==='hasCYL'||k==='hasWomenFederation'">
                        {{form[k]===1?'是':'否'}}
                      </span>
                      <span v-else>
                         {{form[k]}}
                ...
                <tr>
                  ...
                  <td>
                    <img width="200px"  v-for="s in images" :key="s" :src="s" alt="">
                  </td>
                </tr>
                <tr>
                      <td colspan="2" align="center">
                        <el-button type="text" size="large" @click="printTable('deliveryInfo')">[打印]</el-button>
                      </td>
                  </tr>
            </table>
        </el-col>
...
export default {
  data() {
    return {
      ...
      label:['统一社会信用代码','企业名称','企业类别','企业地址','行政区划代码',
      '企业联系方式','企业员工数','法定代表人证件类型','法定代表人证件号码','法定代表人姓名','法定代表人联系方式',
      '保卫负责人身份证','治保负责人姓名','治保负责人联系方式','董事长身份证号','董事长姓名','董事长联系电话','总经理身份证号','总经理姓名','总经理联系方式','是否危化企业','安全隐患类型','关注程度','是否具备建立中共党组织条件','是否有中共党组织',
      '中共党员数量','是否有工会','工会成员数量','是否有共青团','共青团员数量','是否有妇联','妇女数量','经度','纬度','所属网格','业务经营范围'],
      images:[]
    };
  },
  methods:{
...
// WEBPACK FOOTER //
// src/views/publicSecurityManage/CompanyInfo.vue?d843b97a