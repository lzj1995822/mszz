              ...
              <el-col :span="16">
                <div align="right">
                  <el-button type="primary" @click="addEconomyOrg">
                    <i class="el-icon-plus">添加非公有制经济组织</i>
                  </el-button>
...
import info from "./economyOrgInfo";
...
export default {
  ...
  methods: {
    ...
    addEconomyOrg() {
      this.$router.push("addEconomyOrg");
    },
    getCount(data) {
      let sum = 0;
    ...
    initCharts() {
      api.getEconomyHistogram().then(res=>{
        ...
        this.drawHistogram(this.area,this.areaData);
      })
    },
    watchDetail(id) {
      this.id = id;
      this.visible = true;
      // this.$router.push({ path: "economyOrgInfo", query: { id: id } });
    },
    editForm(id) {
      this.$router.push({ path: "editEconomyOrgInfo", query: { id: id } });
    },
    showAll() {
...
// WEBPACK FOOTER //
// src/views/socialOrganizationManage/economyOrgManage.vue?29d5797e